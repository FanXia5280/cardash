...
import com.s05.hudtraffic.HudTrafficService;

/**
 * 往车机桌面的「桌面设置」里挂「HUD 红绿灯」入口（在氛围灯下面，纯运行时挂载）。
 *
 ...
 *
 * <p>同时启动 {@link HudTrafficService} —— 完整保留用户源码里的前台服务（动态注册收
 * 高德广播、往 HUD 副屏叠加显示、发指令激活 HUD 显示、看门狗），跟着桌面进程常驻，
 * 不用单独装 apk，只要 D+ 桌面在运行，HUD 显示就在运行。</p>
 */
public final class HudEntry {
    ...
    /** 自留 id 段（氛围灯 LanternEntry 用 0x0C4D0002，这里用下一个，避开冲突） */
    private static final int ID_HUD = 0x0C4D0003;
    ...
    public static void install(Context context) {
        ...
        // ① 保活：HUD 红绿灯前台服务跟着桌面进程常驻（收高德广播 + 画 HUD + 发指令激活显示）
        try {
            HudTrafficService.start(app);
            Diagnostics.log("HUD 红绿灯服务已启动（跟随桌面进程）");
        } catch (Throwable t) {
            Diagnostics.log("HUD 红绿灯服务启动失败: " + t);
        }
        ...
        try {
            app.registerActivityLifecycleCallbacks(new Application.ActivityLifecycleCallbacks() {
                ...
                public void onActivityResumed(Activity activity) {
                    try {
                        ...
                        Diagnostics.log("HUD 红绿灯设置入口挂载失败: " + t);
                    }
                }
            ...
            Diagnostics.log("HUD 红绿灯设置入口已注册");
        } catch (Throwable t) {
            Diagnostics.log("HUD 红绿灯设置入口注册失败: " + t);
        }
    }
    ...
    private static void attach(Activity a) {
        ...
        group.addView(makeItem(a, group, container, "HUD 红绿灯", ID_HUD));
    ...
    private static RadioButton makeItem(Context c, RadioGroup group, final ViewGroup container,
                                        String label, int id) {
        final RadioButton item = buildItem(c, group, label);
        item.setId(id);
    ...
    private static RadioButton buildItem(Context c, RadioGroup group, String label) {
        ...
        if (tmpl != null) {
            ...
            item.setCompoundDrawables(entryIcon(c, tmpl), null, null, null);
        }
    ...
    /** 入口项图标：代码画的红绿灯（竖排三灯），尺寸跟原厂项一致。 */
    private static Drawable entryIcon(Context c, TextView template) {
        int w = 0;
        int h = 0;
        if (template != null) {
        ...
        layer.setLayerInset(1, w / 4, 1, w / 4, h - lamp - 1);
        layer.setLayerInset(2, w / 4, (h - lamp) / 2, w / 4, (h - lamp) / 2);
        layer.setLayerInset(3, w / 4, h - lamp - 1, w / 4, 1);
