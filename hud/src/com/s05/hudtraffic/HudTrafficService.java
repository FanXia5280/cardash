 ...
 *   <li>保活：AlarmManager 心跳 + 独立进程守护（{@link DaemonService}）+ 无障碍服务。</li>
 * </ul>
 */
public class HudTrafficService extends Service implements DisplayManager.DisplayListener {
    ...
    public void onCreate() {
        ...
        // 悬浮球入口：高德设置页是引擎自绘的插不了菜单，用它当入口（导航中会自动隐藏）
        ...
        AppLog.i("Service", "服务 onCreate 完成（心跳+守护进程已启动）");
    }
    ...
    public void onTaskRemoved(Intent rootIntent) {
        // 从最近任务划掉时：安排心跳尽快把服务重新拉起
        AppLog.i("Service", "任务被划掉，安排心跳立刻拉起");
    ...
    public void onDestroy() {
        ...
        running = false;
        // 只要用户还要 HUD，就安排心跳把服务重新拉起来（START_STICKY 之外的双保险）
        if (Prefs.isHudEnabled(this) || Prefs.isSimulate(this)) {
    ...
    public static void stop(Context ctx) {
        ...
        try {
            app.stopService(new Intent(app, DaemonService.class));
        } catch (Throwable ignored) {
        }
    }
