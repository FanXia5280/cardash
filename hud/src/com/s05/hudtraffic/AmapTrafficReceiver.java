    ...
    private void handle(Intent intent) {
        ...
        if (keyType == TYPE_TRAFFIC_LIGHT) {
            ...
            if (mode != drivingMode) {
                drivingMode = mode;
                currentMode = mode;
                // （独立版里这里会同步悬浮球入口的显隐；内嵌进桌面后没有悬浮球，跳过）
                AppLog.i(TAG, "驾驶模式变化 -> "
                ...
                if (appContext != null) {
                    // 内嵌版：refresh/bounce/wake 都不走前台服务，直接调 HudRuntime 的等价物
                    HudRuntime.refresh(appContext);
                    ...
                    if (mode == 0) {
                        ...
                        //（用户实测手动这么做有效）
                        HudRuntime.bounceHudSwitch(appContext);
                    } else if (mode == 1) {
                        // 开始导航：把自己顶回最上层（D 桌面的歌词窗口可能盖住我们）
                        HudRuntime.wakeHudLater(appContext, 0L);
                        HudRuntime.bringFrontLater(appContext, 1200L);
                    } else {
                        HudRuntime.wakeHudLater(appContext, 1500L);
                    }
                }
            }
